...
- **Ver planilha** (*See worksheet*): opens the spreadsheet on the browser:

 ![window](readme_images/see_worksheet.png)
...
![window](readme_images/sendmessconf1.png)![window](readme_images/sendmessconf2.png)
...
These files are used to authenticate Google Sheets API integration, therefore they are particular to each use case.

**credentials.json** is generated by Google Cloud when activating the API, the file should then be copied to the root of the folder. It has important and sensible data about the project.
```
{
    "installed":
    {
...
**token.json**, on the other hand, is used by the system to check if the user has already been authenticated. The file is create by the software when the user first logs in, it contains the token itself, the client's data and an expiration date.
```
{
    "token": "", 
    "refresh_token": "", 
...
### **windows.py**
Create and manage tkinter windows. Includes the Window class, used to create the tkinter root used by the system, it includes the specifications of design, text contents for components, images setups and all of the window's behavior management.
```
...
class Window:
    def __init__(self):
...
> ***message_box(type, title, message):*** This function generates the corresponding tkinter alert message, according to type. 
```
...
...
## **Credits**
Software planning, designing and development: [Guilherme Moret](https://github.com/Takater) - [Instagram](https://instagram.com/gmoretdev)

Client: [Raynara Santos Nutricionista](https://instagram.com/ray.nutricionista/)